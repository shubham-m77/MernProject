...
const serviceFetch=async()=>{
    try {
        const response=await fetch("http://localhost:2000/auth/services",{
            method:"GET",
        })
        if(response.ok){
  ...
  useEffect(() => {
    ...
    return <AuthContext.Provider value={{storeTokenInLS,LogoutUser,isLoggedIn,uData,service,authToken,isLoading}}>
        {children} 
